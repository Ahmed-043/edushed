...
class RoomTimetable extends StatefulWidget {
  final String title;
  final List<Timetable> timetable;

  RoomTimetable({required this.title, required this.timetable});

  @override
  State<RoomTimetable> createState() => _RoomTimetableState();
}

class _RoomTimetableState extends State<RoomTimetable> {
          ...
          actions: [
            ...
            PopupMenuButton<String>(
              ...
              onSelected: (value) async {
                if (value == 'Optimize') {
                  setState(() {
                    optimize(); // call your optimize function
                                                                      ...
                                                                      child: Container(
                                                                        ...
                                                                        child: Center(
                                                                          child: LayoutBuilder( builder: (context,constraints){
                                                                            ...
                                                                            int sectionNum = widget.timetable[index].slot[d][s].section;
                                                                            String sectionAlpha = String.fromCharCode(64 + sectionNum);
                                                                            return Text("${widget.timetable[index].slot[d][s].subject.isEmpty ? "" :
                                                                                widget.timetable[index].slot[d][s].subject} "
                                                                                "\n ${ widget.timetable[index].slot[d][s].dept.isNotEmpty ? "${widget.timetable[index].slot[d][s].teacher} ${widget.timetable[index].slot[d][s].dept}" "-" "$sectionAlpha" : "" }"
  ...
  void search(var Value) {
    int value = (int.tryParse(Value) ?? -1) -1; // Convert input to int, default to -1 if invalid
    if (value is int && value >= 0 && value < widget.timetable.length) {
      _scrollController.animateTo(
        value * (MediaQuery.of(context).size.height * 0.85 + 20),
  ...
  Future<void> printTimetables(List<Timetable> timetables) async {
    ...
    try {
      ...
      final timetablesDir = Directory('$selectedDir/Room_Timetables');
